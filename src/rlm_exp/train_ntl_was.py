...
from src.utils.rlm_ntl_was import NumberTokenLoss
...
class HuggingFaceRegressionAdapter(torch.nn.Module):
    ...
    def __init__(self, cfg: DictConfig, device: Optional[torch.device] = None):
        ...
        self.NTL = NumberTokenLoss(self.tokenizer, self.device)
        self.to(self.device)
    ...
    def compute_loss_and_metrics(self, batch: Dict):
        ...
        try:
            ...
            ntl_loss = self.NTL.forward(logits, labels_for_loss)
            loss = ce_loss + 0.3 * ntl_loss
            # ======================================================
        ...
        return loss, {
            ...
            "ntl_loss": float(ntl_loss.detach().item()),
...
def main(cfg: DictConfig):
    ...
    #
    base_results_dir = Path(f"results_exp_rlm_ntl_was")
