...
class PyTorchModel(nn.Module, model_base.Model[Tensor]):
    ...
    def __init__(
        ...
        **architecture_kwargs,
        ...
        self.critic = architecture.CustomEncoderDecoder_critic(
            custom_encoder=architecture.MLPEncoder(**architecture_kwargs),
            decoder_vocab_size=len(self.decoder_vocab),
            encoder_pad_idx=self.encoder_vocab.pad_id,
            max_decoder_len=self.decode_len + 1,
    ...
    def compute_loss_and_metrics_with_reinforce(
        ...
        """REINFORCEKL"""
        metrics = reinforce_loss_fn.train_step(
            model=self,
            optimizer=optimizer,
            batch=examples,
    ...
    def decode(
        ...
        for step_idx in range(self.decode_len):
            ...
            # Shape: (B*S, V)
            logits, _ = self.encoder_decoder.next_token_logits(
    ...
    def sample_with_logprobs(
        ...
        # Encode
        memory, memory_key_padding_mask = self.encoder_decoder.encode(encoder_input)
        memory_critic, memory_key_padding_mask_critic = self.critic.encode(encoder_input)
        ...
        expanded_memory_critic = (
            memory.unsqueeze(1)
            .expand(-1, num_samples, -1, -1)
            .reshape(batch_size * num_samples, memory_critic.size(1), memory_critic.size(2))
        )
        if memory_key_padding_mask_critic is not None:
            expanded_memory_key_padding_mask_critic = (
                memory_key_padding_mask_critic.unsqueeze(1)
                .expand(-1, num_samples, -1)
                .reshape(batch_size * num_samples, memory_key_padding_mask_critic.size(1))
            )
        else:
            expanded_memory_key_padding_mask_critic = None
        ...
        # step_critic_values = torch.zeros(...)
        # step_logits = torch.zeros(...)

        #
        gen_token_ids_list = []
        step_log_probs_list = []
        step_critic_values_list = []
        step_logits_list = []
        ...
        for step_idx in range(self.decode_len):
            ...
            critic_value = self.critic.next_token_logits(
                current_tgt_ids, expanded_memory_critic, expanded_memory_key_padding_mask_critic
            )
            ...
            #   Token ID ()
            gen_token_ids_list.append(token_ids.squeeze(-1))

            # tokenlog prob
            chosen_log_prob = torch.gather(log_probs, 1, token_ids).squeeze(1)  # (B*S)

            #
            step_log_probs_list.append(chosen_log_prob)
            step_critic_values_list.append(critic_value[:, 0])
        ...
        #   torch.stack
        # : (B*S, L_decode)
        final_decoded_ids = torch.stack(gen_token_ids_list, dim=1)
        step_log_probs = torch.stack(step_log_probs_list, dim=1)
        step_critic_values = torch.stack(step_critic_values_list, dim=1)
        ...
        #  (Reshape)
        final_decoded_ids = final_decoded_ids.view(batch_size, num_samples, self.decode_len)
        step_log_probs = step_log_probs.view(batch_size, num_samples, self.decode_len)
        step_critic_values = step_critic_values.view(batch_size, num_samples, self.decode_len)
        ...
        return final_decoded_ids, output_floats, step_log_probs, step_logits, step_critic_values
    ...
    def sample_with_logprobs_reject(
        ...
        for step_idx in range(self.decode_len):
            # (B*S, V)
            logits, critic_value = self.encoder_decoder.next_token_logits(
                current_tgt_ids, expanded_memory, expanded_memory_key_padding_mask
            )
    ...
    def log_prob(self, examples: dict[str, Tensor]) -> Tensor:
        ...
        logits, _ = self.encoder_decoder.forward(enc_input, dec_input)
