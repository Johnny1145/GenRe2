...
class PyTorchModel(nn.Module, model_base.Model[Tensor]):
    ...
    def __init__(
        ...
        **architecture_kwargs,
        ...
        self.encoder_decoder = architecture.DatasetEncoderDecoder(
    ...
    def to_device(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        for k, v in batch.items():
            ...
            elif isinstance(v, list) and isinstance(v[0], Tensor):
                batch[k] = [v0.to(self.device) for v0 in v]
        return batch
    ...
    def compute_loss_and_metrics(
        ...
        logits = self.encoder_decoder.forward(
            src=examples["encoder_input"],
            tgt_input=examples["decoder_input"],
            dataset_embedding=examples["dataset_embedding"],
        )
    ...
    def decode(
        ...
        dataset_embedding = inputs["dataset_embedding"]
        device = encoder_input.device
        batch_size = encoder_input.shape[0]
        # memory: (B, L_src, D_model), memory_key_padding_mask: (B, L_src)
        memory, memory_key_padding_mask = self.encoder_decoder.encode(encoder_input, dataset_embedding)
    ...
    def convert_inputs(self, inputs: Sequence[core.ExampleInput]) -> dict[str, Tensor]:
        ...
        dataset_embedding = [example.dataset_embedding for example in inputs]
        # epoch = [example.epoch for example in inputs]
        result = {"encoder_input": torch.tensor(encoder_input),
    ...
    def convert_examples(self, examples: Sequence[core.Example]) -> dict[str, Tensor]:
        ...
        result = self.convert_inputs(examples) | decoder_out
        return result
