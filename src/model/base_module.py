...
class BaseModule:
    ...
    def test_dataset(
        self, test_loader
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict, Dict, Dict, Dict, Dict]:
        """"""
        self.model.eval()
        ...
        for batch in test_loader:
            ...
            decoded_ids, output_floats = self.model.decode_with_mlp_encoder(
                batch, num_samples=128, temperature=1.0  #  128
            )

            #  Greedy
            _, output_floats_greedy = self.model.greedy_decode(
                batch, num_samples=1
            )

            # output_floats
            norm_output_floats, _, _ = normalize_data(output_floats, train_mean, train_std)
            norm_output_floats_greedy, _, _ = normalize_data(output_floats_greedy, train_mean, train_std)
            ...
            norm_output_floats_greedy_val = norm_output_floats_greedy.squeeze(1).squeeze(-1)
            ...
            predictions_greedy = np.array(norm_output_floats_greedy_val).flatten()
            ...
            all_predictions_greedy.extend(predictions_greedy)
        ...
        predictions_greedy = np.array(all_predictions_greedy)
        ...
        metrics_mean = calculate_metrics(norm_target, predictions_mean)
        metrics_median = calculate_metrics(norm_target, predictions_median)
        metrics_clip_mean = calculate_metrics(norm_target, predictions_clip_mean)
        metrics_clip_median = calculate_metrics(norm_target, predictions_clip_median)
        metrics_greedy = calculate_metrics(norm_target, predictions_greedy)

        return (
            predictions_mean,
            predictions_median,
            predictions_clip_mean,
            predictions_clip_median,
            # predictions_greedy,
            norm_target,
            metrics_mean,
            metrics_median,
    ...
    def test_dataset_normalized(
        ...
        for batch in test_loader:
            ...
            # norm_output_floats_greedy, _, _ = normalize_data_normalized(output_floats_greedy, y_max, y_min)
            ...
            output_floats_clip_mean = np.mean(norm_output_floats_clip, axis=1)
            output_floats_clip_median = np.median(norm_output_floats_clip, axis=1)
            # output_floats_greedy_val = output_floats_greedy.squeeze(1).squeeze(-1) # (batch_size,)
            # norm_output_floats_greedy_val = norm_output_floats_greedy.squeeze(1).squeeze(-1)
