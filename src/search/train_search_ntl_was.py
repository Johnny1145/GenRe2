...
class RegressionModule(BaseModule):
    ...
    def train_epoch(self) -> Dict[str, float]:
        self.model.train()
        total_loss = 0
        self.NTL = NumberTokenLoss_WAS(self.model.decoder_vocab, self.model.device)
...
def main(cfg: DictConfig):
    ...
    results_dir = Path(f"results_search_mlp_encoder_ntl_was/{dataset_name}")
    results_dir.mkdir(parents=True, exist_ok=True)
    # results.json
    if cfg.skip_mode:
        results_json_path =Path(f"results_search_mlp_encoder_ntl_was/{dataset_name}/{dataset_name}/results_seed_{cfg.seed}.json")
