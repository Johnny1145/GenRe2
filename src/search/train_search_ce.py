...
class RegressionModule(BaseModule):
    ...
    def fit(
        ...
        for epoch in range(num_epochs):
            ...
            # epochreinforce_weight1.0
            # epochnum_epochsreinforce_weight1
...
def train_and_test_single_task(
    ...
    # median_dir = Path(f"results_exp5_P10_mlp_encoder_ce/{dataset_name}")
    # median_load = str(median_dir / dataset_name / "checkpoints")
    # ckpt_path = median_load + "/checkpoint_best/model.pt"
    ...
    logger.info(
        f" {dataset_name}  - MSE_mean: {metrics_mean['mse']:.6f}, Rank Corr_mean: {metrics_mean['rank_correlation']:.6f}"
    )
    logger.info(
...
def main(cfg: DictConfig):
    ...
    # # results.json
    if cfg.skip_mode:
        results_json_path = results_dir / dataset_name / f"checkpoints_{cfg.seed}" / "checkpoint_best" / "metrics.json"
